...
namespace
{
  ...
  [[maybe_unused]] void TestNumbers()
  {
    ...
    auto root = LoadJSON("123.45"s).GetRoot();
    assert(root == dbl_node);
  ...
  [[maybe_unused]] void TestArray()
  {
    ...
    auto arr_node_clone = LoadJSON(Print(arr_node)).GetRoot();
    assert(arr_node_clone == arr_node);
  ...
  [[maybe_unused]] void Benchmark()
  {
    ...
    Document _doc{arr};
    json::Print(_doc, strm);
    std::string strm_str = strm.str();
